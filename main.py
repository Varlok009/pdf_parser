...
if __name__ == '__main__':
    # Для простоты отладки можно вписать пусть к файлу ниже, иначе подтянется значение введенное в командной строке
    script_name, file_path = (argv[0], argv[1]) if len(argv) == 2 else (argv, 'files/standart.pdf')

    f = PdfFile(file_path)

    print(f'Словарь с параметрами файла - {f.get_params_from_pdf()}')
